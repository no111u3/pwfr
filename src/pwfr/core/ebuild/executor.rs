...
use crate::core::ebuild::efile::{
    ...
    Initializer, LocalDeclaration, ProcSubstType, RunIf, Span, Word,
};
...
impl Job {
    ...
    pub fn state(&self, executor: &Executor) -> &'static str {
        if self.completed(executor) {
            "done"
        } else if self.stopped(executor) {
            "stopped"
        } else {
            "running"
        }
    ...
    pub fn completed(&self, executor: &Executor) -> bool {
        self.processes.iter().all(|pid| {
            let state = executor.get_process_state(*pid).unwrap();
            match state {
    ...
    pub fn stopped(&self, executor: &Executor) -> bool {
        self.processes.iter().all(|pid| {
            let state = executor.get_process_state(*pid).unwrap();
            match state {
...
impl Executor {
    ...
    fn evaluate_expr(&mut self, expr: &Expr) -> i32 {
        match expr {
            ...
            Expr::Parameter { name } => self.get_var_as_i32(&name).unwrap_or(0),
            Expr::Add(BinaryExpr { lhs, rhs }) => self.evaluate_expr(lhs) + self.evaluate_expr(rhs),
            Expr::Sub(BinaryExpr { lhs, rhs }) => self.evaluate_expr(lhs) - self.evaluate_expr(rhs),
            Expr::Mul(BinaryExpr { lhs, rhs }) => self.evaluate_expr(lhs) * self.evaluate_expr(rhs),
            Expr::Div(BinaryExpr { lhs, rhs }) => self.evaluate_expr(lhs) / self.evaluate_expr(rhs),
            Expr::Assign { name, rhs } => {
                let value = self.evaluate_expr(rhs);
                self.assign(&name, Value::String(value.to_string()));
                value
            }
            Expr::Eq(BinaryExpr { lhs, rhs }) => {
                bool_to_int!(self.evaluate_expr(lhs) == self.evaluate_expr(rhs))
            }
            Expr::Ne(BinaryExpr { lhs, rhs }) => {
                bool_to_int!(self.evaluate_expr(lhs) != self.evaluate_expr(rhs))
            }
            Expr::Lt(BinaryExpr { lhs, rhs }) => {
                bool_to_int!(self.evaluate_expr(lhs) < self.evaluate_expr(rhs))
            }
            Expr::Le(BinaryExpr { lhs, rhs }) => {
                bool_to_int!(self.evaluate_expr(lhs) <= self.evaluate_expr(rhs))
            }
            Expr::Gt(BinaryExpr { lhs, rhs }) => {
                bool_to_int!(self.evaluate_expr(lhs) > self.evaluate_expr(rhs))
            }
            Expr::Ge(BinaryExpr { lhs, rhs }) => {
                bool_to_int!(self.evaluate_expr(lhs) >= self.evaluate_expr(rhs))
            }
            Expr::Inc(name) => {
                ...
                if let Some(var) = self.get(name) {
                    ...
                    match (op, value) {
                        ...
                        (
                            ...
                            let replaced =
                                self.replace_pattern(pattern, &content, replacement, *replace_all)?;
    ...
    fn expand_word_into_string(&mut self, word: &Word) -> Result<String> {
        let ws: Vec<String> = self
            .expand_word_into_vec(word, &self.ifs())?
            .into_iter()
            .map(|w| w.into_string())
            .collect();
    ...
    fn evaluate_initializer(&mut self, initializer: &Initializer) -> Result<Value> {
        match initializer {
            Initializer::String(ref word) => Ok(Value::String(self.expand_word_into_string(word)?)),
    ...
    pub fn run_in_foreground(&mut self, job: &Arc<Job>, sigcont: bool) -> ProcessState {
        ...
        if self.tcsetpgrp_enabled {
            job.termios
                .replace(Some(tcgetattr(self.term_fd).expect("failed to tcgetattr")));
        }
    ...
    fn run_external_command(
        ...
        ctx: &Context,
        argv: Vec<String>,
        redirects: &[efile::Redirection],
        assignments: &[efile::Assignment],
    ) -> Result<ExitStatus> {
        let mut fds = Vec::new();
        for r in redirects {
            match r.target {
                efile::RedirectionType::Fd(ref fd) => {
                    fds.push((*fd, r.fd as RawFd));
                }
                efile::RedirectionType::File(ref wfilepath) => {
                    let mut options = OpenOptions::new();
                    match &r.direction {
                        efile::RedirectionDirection::Input => {
                            options.read(true);
                        }
                        efile::RedirectionDirection::Output => {
                            options.write(true).create(true);
                        }
                        efile::RedirectionDirection::Append => {
                ...
                efile::RedirectionType::HereDoc(ref heredoc) => {
                    fds.push((self.evaluate_heredoc(heredoc)?, r.fd as RawFd))
                }
                efile::RedirectionType::UnresolvedHereDoc(_) => {
                    // must be resolved in the efile
                    unreachable!()
                }
            }
        }
        ...
        let argv0 = if argv[0].starts_with('/') || argv[0].starts_with("./") {
            ...
            match lookup_external_command(&argv[0]) {
                ...
                None => {
                    eprintln!("Ebuild: command not found `{}'", argv[0]);
                    return Ok(ExitStatus::ExitedWith(1));
                }
            }
        };
    ...
    pub fn run_internal_command(
        ...
        for r in redirects {
            match r.target {
                efile::RedirectionType::Fd(ref fd) => match r.fd {
                    0 => stdin = *fd,
                    1 => stdout = *fd,
                    2 => stderr = *fd,
                    _ => (),
    ...
    /// TODO: Aliases should be expanded in the efile in order to support
        ...
        match result {
            ...
            Err(err) => {
                match err.find_root_cause().downcast_ref::<InternalCommandError>() {
                    Some(InternalCommandError::BadRedirection) => {
                        return Ok(ExitStatus::ExitedWith(1))
                    }
        ...
        if self.in_global_frame() {
            ...
            for decl in declarations {
                match decl {
                    ...
                    LocalDeclaration::Name(name) => self.define(name, true),
                }
            }
    ...
    fn run_if_command(
        &mut self,
        ctx: &Context,
        condition: &[efile::Term],
        then_part: &[efile::Term],
        elif_parts: &[efile::ElIf],
        else_part: &Option<Vec<efile::Term>>,
        _redirections: &[efile::Redirection],
    ) -> Result<ExitStatus> {
        // then
    ...
    fn run_arith_for_command(
        ...
        'for_loop: while self.evaluate_expr(cond) == 1 {
            let result = self.run_terms(body, ctx.stdin, ctx.stdout, ctx.stderr);
            match result {
                ExitStatus::Break => break 'for_loop,
                ExitStatus::Continue => (),
    ...
    fn run_command(&mut self, command: &efile::Command, ctx: &Context) -> Result<ExitStatus> {
        if self.noexec {
        ...
        let result = match command {
            ...
            efile::Command::While { condition, body } => {
                self.run_while_command(ctx, &condition, &body)?
            }
            efile::Command::Case { word, cases } => self.run_case_command(ctx, &word, &cases)?,
            efile::Command::For {
                var_name,
                words,
                body,
            } => self.run_for_command(ctx, var_name, &words, &body)?,
            efile::Command::ArithFor {
                init,
                cond,
                update,
                body,
            } => self.run_arith_for_command(ctx, init, cond, update, &body)?,
            efile::Command::LocalDef { declarations } => self.run_local_command(&declarations)?,
            efile::Command::FunctionDef { name, body } => {
                self.set(name, Value::Function(body.clone()), true);
                ExitStatus::ExitedWith(0)
            }
            efile::Command::Assignment { assignments } => {
            ...
            efile::Command::Break => ExitStatus::Break,
            efile::Command::Continue => ExitStatus::Continue,
        };
    ...
    pub fn eval_in_subshell(&mut self, terms: &[efile::Term]) -> Result<(i32, i32)> {
        let (pipe_out, pipe_in) = pipe().expect("failed to create a pipe");
    ...
    fn spawn_subshell(&mut self, terms: &[efile::Term], ctx: &Context) -> Result<Pid> {
    ...
    pub fn run_terms(
        ...
        for term in terms {
            for pipeline in &term.pipelines {
                ...
                last_status =
                    self.run_pipeline(&term.code, pipeline, stdin, stdout, stderr, term.background);
            }
        }
    ...
    pub fn evaluate_cond_primary(&mut self, cond: &CondExpr) -> Result<String> {
        match cond {
            CondExpr::Word(word) => self.expand_word_into_string(word),
    ...
    pub fn evaluate_cond(&mut self, cond: &CondExpr) -> Result<bool> {
        ...
        macro_rules! parse_as_int {
            ($expr:expr) => {
                self.evaluate_cond_primary($expr)?.parse().unwrap_or(0) as i32
            };
        }

        let result = match cond {
            CondExpr::And(BinaryCondExpr { lhs, rhs }) => eval_as_bool!(lhs) && eval_as_bool!(rhs),
            CondExpr::Or(BinaryCondExpr { lhs, rhs }) => eval_as_bool!(lhs) || eval_as_bool!(rhs),
            CondExpr::StrEq(BinaryCondExpr { lhs, rhs }) => {
                let pat = self.expand_into_single_pattern_word(unwrap_word!(rhs.as_ref()))?;
                match_pattern_all(&pat, &eval_as_string!(lhs))
            }
            CondExpr::StrNe(BinaryCondExpr { lhs, rhs }) => {
                let pat = self.expand_into_single_pattern_word(unwrap_word!(rhs.as_ref()))?;
                !match_pattern_all(&pat, &eval_as_string!(lhs))
            }
            CondExpr::Eq(BinaryCondExpr { lhs, rhs }) => parse_as_int!(lhs) == parse_as_int!(rhs),
            CondExpr::Ne(BinaryCondExpr { lhs, rhs }) => parse_as_int!(lhs) != parse_as_int!(rhs),
            CondExpr::Lt(BinaryCondExpr { lhs, rhs }) => parse_as_int!(lhs) < parse_as_int!(rhs),
            CondExpr::Le(BinaryCondExpr { lhs, rhs }) => parse_as_int!(lhs) <= parse_as_int!(rhs),
            CondExpr::Gt(BinaryCondExpr { lhs, rhs }) => parse_as_int!(lhs) > parse_as_int!(rhs),
            CondExpr::Ge(BinaryCondExpr { lhs, rhs }) => parse_as_int!(lhs) >= parse_as_int!(rhs),
    ...
    /// Runs commands.
    pub fn eval(&mut self, ast: &Ast, stdin: RawFd, stdout: RawFd, stderr: RawFd) -> ExitStatus {
        trace!("ast: {:#?}", ast);
    ...
    pub fn run_str_with_stdio(
        ...
        match efile::parse(script) {
            Ok(ast) => self.eval(&ast, stdin, stdout, stderr),
            Err(efile::ParseError::Empty) => {
                // Just ignore.
                ExitStatus::ExitedWith(0)
            }
...
mod tests {
    ...
    #[test]
    fn test_expr() {
        let mut executor = Executor::new("item one");
        assert_eq!(
            executor.evaluate_expr(&&Expr::Mul(BinaryExpr {
        ...
        executor.set("x", Value::String(3.to_string()), false);
        assert_eq!(
            executor.evaluate_expr(&&Expr::Add(BinaryExpr {
