...
pub enum CondExpr {
    ...
    And(BinaryCondExpr),
    // = or ==
    StrEq(BinaryCondExpr),
    // !=
    StrNe(BinaryCondExpr),
    // -eq
    Eq(BinaryCondExpr),
    Ne(BinaryCondExpr),
    Lt(BinaryCondExpr),
...
pub enum Command {
    SimpleCommand {
        ...
        /// Assignment prefixes. (e.g. "SYS_ENV=some system env")
...
#[derive(Debug, PartialEq, Eq, Clone)]
pub struct BinaryCondExpr {
    pub lhs: Box<CondExpr>,
    pub rhs: Box<CondExpr>,
}
...
pub enum Expr {
    ...
    // Conditions. Evaluated to 1 if it satisfies or 0 if not.
    Eq(BinaryExpr),
    Ne(BinaryExpr),
    Lt(BinaryExpr),
    Le(BinaryExpr),
...
impl EFileParser {
    ...
    fn visit_cond_term(&mut self, pair: Pair<Rule>) -> Box<CondExpr> {
        ...
        if let Some(op) = inner.next() {
            ...
            match op.as_span().as_str() {
                "-eq" => Box::new(CondExpr::Eq(BinaryCondExpr { lhs, rhs })),
                "-ne" => Box::new(CondExpr::Ne(BinaryCondExpr { lhs, rhs })),
                "-lt" => Box::new(CondExpr::Lt(BinaryCondExpr { lhs, rhs })),
                "-le" => Box::new(CondExpr::Le(BinaryCondExpr { lhs, rhs })),
                "-gt" => Box::new(CondExpr::Gt(BinaryCondExpr { lhs, rhs })),
                "-ge" => Box::new(CondExpr::Ge(BinaryCondExpr { lhs, rhs })),
                "==" | "=" => Box::new(CondExpr::StrEq(BinaryCondExpr { lhs, rhs })),
                "!=" => Box::new(CondExpr::StrNe(BinaryCondExpr { lhs, rhs })),
                _ => unimplemented!(),
            }
        } else {
            lhs
    ...
    fn visit_cond_and(&mut self, pair: Pair<Rule>) -> Box<CondExpr> {
        ...
        if let Some(rhs) = inner.next() {
            ...
            Box::new(CondExpr::And(BinaryCondExpr { lhs, rhs }))
        } else {
            lhs
        }
    }
    ...
    fn visit_cond_or(&mut self, pair: Pair<Rule>) -> Box<CondExpr> {
        ...
        if let Some(rhs) = inner.next() {
            ...
            Box::new(CondExpr::Or(BinaryCondExpr { lhs, rhs }))
        } else {
            lhs
        }
    }
    ...
    pub fn parse(&mut self, script: &str) -> Result<Ast, ParseError> {
        match EFile::parse(Rule::efile, script) {
            Ok(mut pairs) => {
    ...
    pub fn parse_and_dump(&mut self, script: &str) {
        ...
        let pairs = EFile::parse(Rule::efile, &merged_script).unwrap_or_else(|e| panic!("{}", e));
        self.dump(pairs, 0);
    }
}

pub fn parse(efile: &str) -> Result<Ast, ParseError> {
    let mut parser = EFileParser::new();
    parser.parse(efile)
}
...
mod tests {
    ...
    #[test]
    fn tree_comments_and_variables() {
        let efile = EFile::parse(
            Rule::efile,
            "a = \"4\" #some comment\nb  =\"6\"#another comment\nc= \"7\"  #other comment\nd=\"8\"",
        )
                ...
                terms: vec![Term {
                    ...
                    pipelines: vec![Pipeline {
                        ...
                        commands: vec![Command::ArithFor {
                            ...
                            cond: Expr::Lt(BinaryExpr {
                                lhs: Box::new(Expr::Parameter { name: "i".into() }),
    ...
    #[test]
    fn ast_test_process_substitution() {
        assert_eq!(
                    ...
                    Term {
                        ...
                        pipelines: vec![Pipeline {
                            ...
                            commands: vec![Command::Cond(Box::new(CondExpr::StrEq(
                                BinaryCondExpr {
                                    lhs: Box::new(CondExpr::Word(Word(vec![Span::Parameter {
                                        name: "hello".into(),
                                        op: ExpansionOp::GetOrEmpty,
                                        quoted: false,
                                    }]))),
                                    rhs: Box::new(CondExpr::Word(lit!("world"))),
                                }
                            )))],
                        }],
                    },
